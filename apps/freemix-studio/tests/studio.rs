...
use fm_protocol::{
    ...
    FadeToBlackState, HandshakeOutcome, HandshakeResponse, LineDecoder,
    MANUAL_ALPHA_FADE_PROTOCOL_VERSION, ManualTransitionStatus, ProtocolVersion, Role,
...
fn serve_snapshot_then_resume(listener: &TcpListener) {
    ...
    assert_eq!(request.versions, vec![MANUAL_ALPHA_FADE_PROTOCOL_VERSION]);
